                ...
                for (int i = 0; i < len; i++) {
                    ...
                    if (nonCSSPresentationalHints.contains(an)) {
                        try {
                            ...
                            if (nodeValue.startsWith("rgb")) {
                                String[] colorArray = nodeValue.substring(nodeValue.indexOf("(") + 1, nodeValue.length() - 1).split(",");
                                int red = Integer.parseInt(colorArray[0]);
                                int green = Integer.parseInt(colorArray[1]);
                                int blue = Integer.parseInt(colorArray[2]);
                                if (colorArray.length > 3 && !elt.hasAttribute("opacity")) {
                                    elt.setAttribute("opacity", colorArray[3]);
                                }
                                nodeValue = ColorUtil.toHexString(new Color(red, green, blue));
                            }
                            lu = parser.parsePropertyValue(nodeValue);
