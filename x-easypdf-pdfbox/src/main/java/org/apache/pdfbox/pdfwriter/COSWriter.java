...
public class COSWriter implements ICOSVisitor {
     ...
     */
    public static final byte[] VERSION = "PDF-1.6".getBytes(StandardCharsets.US_ASCII);
    /**
    ...
    protected final Map<COSBase, COSObjectKey> objectKeys = new Hashtable<>();
    protected final Map<COSObjectKey, COSBase> keyObject = new HashMap<>();
    // the list of x ref entries to be made so far
    protected final List<XReferenceEntry> xRefEntries = new ArrayList<>();
    // A list of objects to write.
    protected final Deque<COSBase> objectsToWrite = new ArrayDeque<>();
    ...
    protected COSObjectKey currentObjectKey = null;
    ...
    private void addObjectToWrite(COSBase object) {
        ...
        // if (writtenObjects.contains(object) || actualsAdded.contains(actual) || objectsToWrite.contains(object)) {
        //     return;
        // }
        if (object.isWritten() || actual.isNotObject() || objectsToWrite.contains(object)) {
            return;
        }
    ...
    protected void doWriteHeader(COSDocument doc) throws IOException {
        if (isCompress()) {
            pdDocument.setVersion(Math.max(pdDocument.getVersion(), COSWriterCompressionPool.MINIMUM_SUPPORTED_VERSION));
            doc.setVersion(Math.max(doc.getVersion(), COSWriterCompressionPool.MINIMUM_SUPPORTED_VERSION));
        }
        String headerString;
        if (fdfDocument != null) {
    ...
    private void fillGapsWithFreeEntries() {
        ...
        for (int i = 0; i < numberOfFreeNumbers - 1; i++) {
            addXRefEntry(new FreeXReference(new COSObjectKey(freeNumbers.get(i), 65535), freeNumbers.get(i + 1)));
        }
        // add free entry for the last one referencing object 0 as next free one
        addXRefEntry(new FreeXReference(new COSObjectKey(freeNumbers.get(numberOfFreeNumbers - 1), 65535), 0));
    ...
    private void doWriteSignature2() throws IOException {
    ...
    private COSObjectKey getObjectKey(COSBase obj) {
        ...
        if (key == null || !key.equals(actualKey)) {
        ...
        if (incrementalUpdate) {
            trailer.toIncrement().exclude(trailer).forEach(base -> {
                ...
                if (base instanceof COSObject) {
                    ...
                    ((COSObject) base).getObject().setNotObject(true);
                } else {
                    // actualsAdded.add(base);
                    base.setNotObject(true);
                }
            });
        }
