...
public class COSWriterCompressionPool {
    ...
    private COSBase addObjectToPool(COSObjectKey key, COSBase base) {
        ...
        if ((key != null && key.getGeneration() != 0) || current instanceof COSStream || (document.getEncryption() != null && current == document.getEncryption().getCOSObject()) || current == this.document.getDocument().getTrailer().getCOSDictionary(COSName.ROOT)) {
