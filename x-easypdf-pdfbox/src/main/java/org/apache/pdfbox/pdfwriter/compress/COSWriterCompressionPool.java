...
public class COSWriterCompressionPool
{
    ...
    private COSBase addObjectToPool(COSObjectKey key, COSBase base)
    {
        ...
        // types as well
        if (current == null || (key != null && objectPool.contains(key)) || (key == null && objectPool.contains(current)))
        {
            return current;
        }
    ...
    private void addElements(Iterator<COSBase> elements) throws IOException
    {
        while (elements.hasNext())
        {
            ...
            if (value instanceof COSArray || (value instanceof COSDictionary && !value.isAllDirect()))
            {
                // allDirectObjects.add(value);
                value.setAllDirect(true);
                addStructure(value);
            }
