...
public class PDComplexFileSpecification extends PDFileSpecification {
    ...
    public PDEmbeddedFile getEmbeddedFile() {
        PDEmbeddedFile embeddedFile = getEmbeddedFileUnicode();
        if (Objects.nonNull(embeddedFile)) {
            return embeddedFile;
        }
        embeddedFile = getEmbeddedFileDos();
        if (Objects.nonNull(embeddedFile)) {
            return embeddedFile;
        }
        embeddedFile = getEmbeddedFileMac();
        if (Objects.nonNull(embeddedFile)) {
            return embeddedFile;
        }
        embeddedFile = getEmbeddedFileUnix();
        if (Objects.nonNull(embeddedFile)) {
            return embeddedFile;
        }
