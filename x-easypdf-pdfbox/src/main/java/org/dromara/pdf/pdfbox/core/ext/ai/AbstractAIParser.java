...
public abstract class AbstractAIParser extends AbstractParser {
    ...
    public AbstractAIParser(Document document, String ak, String sk, boolean isJsonResponse) {
        ...
        this.config = new AIParserConfig(
                ak,
                sk,
                "<role>你是一个图像分析与数据提取专家</role>",
                "<role>你是一个数据分析与提取专家</role>",
                isJsonResponse,
                HttpUtil.CLIENT
        );
    }
    ...
    protected AIParseInfo parseInfo(String url, String model, List<Message> messages) {
        // 请求头
        Headers headers = new Headers.Builder().add("Authorization", this.getAuthorization()).build();
        // json体
        ...
        // 响应体
        JSONObject response = HttpUtil.request(this.config.getClient(), url, headers, body);
    ...
    protected String getPageText(int beginPageIndex, int endPageIndex) {
        ...
        return "<task>阅读下面文章，并根据文章内容回答问题</task>" +
                       "<article>" + stripper.getText(this.getDocument()) + "</article>";
    }
