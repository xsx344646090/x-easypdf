 ...
 */
@Getter
public class BookmarkAnalyzer extends AbstractAnalyzer {
     ...
     * 书签信息列表
     */
    private final List<BookmarkInfo> bookmarkInfoList = new ArrayList<>(16);
        ...
        if (Objects.nonNull(documentOutline)) {
            ...
            if (Objects.nonNull(bookmarkIndex) && bookmarkIndex.length > 0) {
                ...
                for (int index : bookmarkIndex) {
                    ...
                    if (index > 0) {
                        // 处理书签
                        this.processOutlineItem(this.bookmarkInfoList, documentCatalog, items.get(index));
                    }
                }
            } else {
                // 遍历pdfbox书签列表
                documentOutline.children().forEach(outlineItem -> this.processOutlineItem(this.bookmarkInfoList, documentCatalog, outlineItem));
            }
        }
    }
     ...
     */
    protected void processOutlineItem(List<BookmarkInfo> list, PDDocumentCatalog documentCatalog, PDOutlineItem outlineItem) {
        // 如果书签为空，则跳过
     ...
     * @return 返回书签列表
     */
    protected List<BookmarkInfo> processOutlineItemChildren(PDDocumentCatalog documentCatalog, PDOutlineItem outlineItem) {
        // 有子书签
        if (outlineItem.hasChildren()) {
            // 定义书签列表
            List<BookmarkInfo> children = new ArrayList<>();
            // 获取子书签
    ...
    protected BookmarkInfo processPageDestination(PDOutlineItem outlineItem, PDDestination destination) {
        ...
        if (destination instanceof PDPageDestination) {
            ...
            if (destination instanceof PDPageXYZDestination) {
                ...
                // 设置颜色
                info.setColor(new Color(outlineItem.getTextColor().toRGB()));
                // 设置是否粗体
     ...
     * @param list 书签列表
     */
    protected void processBookmarkInfo(List<BookmarkInfo> list) {
        // 如果书签列表不为空，则处理书签
        if (!list.isEmpty()) {
            // 获取pdfbox页面树
