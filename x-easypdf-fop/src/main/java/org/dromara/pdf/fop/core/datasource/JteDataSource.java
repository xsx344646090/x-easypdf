...
public class JteDataSource extends AbstractDataSource {
     ...
     * @return 返回jte数据源
     */
    public JteDataSource setTemplatePath(String templatePath) {
     ...
     * @return 返回jte数据源
     */
    public JteDataSource setTemplateData(Map<String, Object> templateData) {
    ...
    private TemplateEngine getTemplateEngine() {
        // 获取引擎
        TemplateEngine engine = JteDataSource.ENGINE_CACHE.get(this.templatePath);
        // 如果引擎为空，则创建引擎
        if (engine == null) {
            try {
                // 加锁
                LOCK.lock();
                // 再次获取引擎
                engine = JteDataSource.ENGINE_CACHE.get(this.templatePath);
                // 如果引擎仍然为空，则创建引擎
                if (engine == null) {
                    // 定义解析器
                    ...
                    // 添加引擎缓存
                    JteDataSource.ENGINE_CACHE.put(this.templatePath, engine);
                }
            } finally {
                // 解锁
                LOCK.unlock();
