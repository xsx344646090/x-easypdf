...
public class SplitLine implements Component {
    ...
    public Element createLeader(Document document) {
        ...
        Optional.ofNullable(this.param.getLength()).ifPresent(
                v -> leader.setAttribute(TemplateAttributes.LEADER_LENGTH, v.intern().toLowerCase())
        );
        // 设置分割线样式（默认：solid）
        Optional.ofNullable(this.param.getStyle()).ifPresent(
                v -> {
                    ...
                    if ("dotted".equalsIgnoreCase(v)) {
                        // 设置虚线样式
                        leader.setAttribute(TemplateAttributes.LEADER_PATTERN, Constants.DEFAULT_DOTTED_SPLIT_LINE_STYLE_VALUE);
                    }
                    // 否则设置为规则样式
                    else {
                        // 设置分割线样式
                        leader.setAttribute(TemplateAttributes.LEADER_PATTERN, Constants.DEFAULT_SPLIT_LINE_STYLE_VALUE);
                        // 设置规则样式
                        leader.setAttribute(TemplateAttributes.RULE_STYLE, v.intern().toLowerCase());
                    }
                }
        );
        // 设置分割线颜色
        Optional.ofNullable(this.param.getColor()).ifPresent(v -> leader.setAttribute(TemplateAttributes.COLOR, v.intern().toLowerCase()));
        // 返回leader元素
        return leader;
    }
}
